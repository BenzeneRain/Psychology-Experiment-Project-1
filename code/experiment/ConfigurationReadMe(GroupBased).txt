CONFIGURATION README FOR GROUP BASED EXPERIMENT
...
Line 8 + ts: Number of conditions groups, let us denote it by cgs. 
The reason to have this is because in each section, conditions in different 
condition groups cannot be mixed. When Initializing the conditions, I 
first shuffle the order of condition groups, and then shuffle the order of
the conditions in each condition group.

Line (8 + ts + 1) ~ (8 + ts + cgs): cgs lines of filenames
Each filename contains the description of one condition group.


PART 2: CONDITION GROUP DESCRIPTION FILE

Line 1: Number of conditions in this condition group. Let us denote
it by cs.

part 2.1: Format of condition description
